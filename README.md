...
The dashboard caches frequently-accessed DOM elements on startup to reduce query overhead in high-frequency update loops.

- **Hot paths cached:** clock updates, throughput, countdown, loading overlay, live counters, AIS status indicators.
- **Impact target:** substantial reduction in repetitive `getElementById` calls in hot loops.
- **Behavior:** if a cached node is detached/replaced, cache access safely re-queries and refreshes the reference.
